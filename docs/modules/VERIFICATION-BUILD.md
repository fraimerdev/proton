...
is published from the dashboard over the bus, not by a slash command.
...
| `service.ts` | The two bus events: post/edit the panel, and grant the role after a website pass. |
...
- **`packages/core`**: `verification.panel_requested` and `verification.web_passed` in `EVENT_TYPES`
  *and* `SERVICE_EMITTED_EVENT_TYPES`; `verify-link.ts` (HMAC-SHA256 over base64url claims, constant-
  time compare, 15-minute expiry).
- **`apps/api`**: two routes behind the shared secret that publish those events. They refuse with a
  named 503 when the bus is absent — the api boots without Redis on purpose, and a 200 here would tell
  an admin the panel was posted when nothing was.
- **`apps/dashboard`**: a public `/verify/$token` route; a "Post the panel" module panel; the
  `postVerificationPanel` and `completeWebVerification` server functions; `VERIFY_LINK_SECRET`.
...
| Panel identity encoded in `custom_id` | Kept, and the panel message id is remembered in Redis besides | The id is what lets a second Post edit rather than post a twin. The `custom_id` still carries no message id, so a press re-reads live config. |
